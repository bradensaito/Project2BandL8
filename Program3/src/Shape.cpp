...
//////////Normals computation
		for (int j = 0; j < posBuf.size(); j += 3)
		{
			vec3 totalNorms = { 0, 0, 0 };

			for (int i = 0; i < eleBuf.size(); i += 3)
			{
				//cerr << eleBuf[i] << " " << eleBuf[i + 1] << " " << eleBuf[i + 2] << endl;

				if (count == eleBuf[i] || count == eleBuf[i + 1] || count == eleBuf[i + 2])
				{
					triangles.push_back(eleBuf[i]);
					triangles.push_back(eleBuf[i + 1]);
					triangles.push_back(eleBuf[i + 2]);
				}
			}

			for (int i = 0; i < triangles.size(); i += 3)
			{
				vec3 p1 = {posBuf[triangles[i] * 3], posBuf[(triangles[i] * 3) + 1], posBuf[(triangles[i]* 3) + 2]};
				vec3 p2 = { posBuf[triangles[i+1] * 3], posBuf[(triangles[i+1] * 3) + 1], posBuf[(triangles[i+1] * 3) + 2] };
				vec3 p3 = { posBuf[triangles[i+2] * 3], posBuf[(triangles[i+2] * 3) + 1], posBuf[(triangles[i+2] * 3) + 2] };

				vec3 v1min2 = p1 - p2;
				vec3 v3min2 = p3 - p2;

				vec3 crossP = cross(v3min2, v1min2);

				totalNorms = totalNorms + crossP;
			}

			totalNorms = normalize(totalNorms);
			norBuf.push_back(totalNorms.x);
			norBuf.push_back(totalNorms.y);
			norBuf.push_back(totalNorms.z);
