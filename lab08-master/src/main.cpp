...
public:
	...
	void render()
	{
		...
		if (mouseDown == true)
		{
			if (oldMouseDown == false)
			{
				glfwGetCursorPos(windowManager->getHandle(), &xPosition, &yPosition);
				oldMouseDown = true;
			}
			else
			{
				glfwGetCursorPos(windowManager->getHandle(), &newX, &newY);
				deltaX = newX - xPosition;
				deltaY = newY - yPosition;
				deltaX += lastX;
				deltaY += lastY;
				cTheta = deltaX * 0.01;
				phi = deltaY * 0.01;
		...
		View->pushMatrix();
		float x = 4.0f * radians(cos(cTheta));
		float y = - 4.0f * radians(sin(phi));
		float z = 4.0f * radians(sin(cTheta));

		View->lookAt(vec3(0, 0, 0), vec3(x, y, z), vec3(0, 1, 0));
